...
- [x] **Phase F — Teacher dashboard integration:** assignment creation (prompt, budget, dial with default-fade prefill), roster with per-cycle TAU chips + flag markers, student conversation view (trajectory strip, conversation-ready moments, snapshots verbatim, transcripts in teacher mode with events inline), teacher note per submission surfaced on the student's report. *Remaining: integrate/retire the mock triage dashboard (teacher-dashboard.html), class-level instructional view (backlog #3)*
